...
REPO_NAME="test-audit"
BINARY="test-audit"
INSTALL_DIR="${TEST_AUDIT_INSTALL_DIR:-$HOME/.test-audit/bin}"
VERSION="${TEST_AUDIT_VERSION:-}"
...
echo -e "  ${RED}   █  ██▄ ▄█  █    █▀█ █▄█ █▄▀ █  █  ${NC}"
echo -e "${NC}"
echo -e "  ${DIM}deterministic certification practice-test auditor${NC}"
...
case "$OS" in
    Linux)  OS="linux" ;;
    Darwin) OS="darwin" ;;
    MINGW*|MSYS*|CYGWIN*) fail "Windows is not supported. Use: go install github.com/${REPO_OWNER}/${REPO_NAME}@latest" ;;
...
if [[ "$INSTALLED" == "false" ]]; then
    if command -v go &>/dev/null; then
        ...
        GOBIN="$INSTALL_DIR" go install "github.com/${REPO_OWNER}/${REPO_NAME}@latest"
        INSTALLED=true
        info "Installed to ${INSTALL_DIR}/${BINARY}"
    else
        echo ""
...
if [[ "$PATH_UPDATED" == "false" ]]; then
    ...
    case "$CURRENT_SHELL" in
        ...
        fish)
            ...
            echo "set -gx PATH \"$INSTALL_DIR\" \$PATH" > "$HOME/.config/fish/conf.d/test-audit.fish"
            info "Added to ~/.config/fish/conf.d/test-audit.fish"
            PATH_UPDATED=true
            ;;
    esac
...
echo ""
echo -e "  ${GREEN}${BOLD}test-audit installed successfully${NC}"
echo ""

if ! command -v test-audit &>/dev/null; then
...
echo ""
echo -e "    ${CYAN}test-audit cissp_test_6.js${NC}        Audit one test -> cissp_test_6_audit.json"
echo -e "    ${CYAN}test-audit *.js -o ./audit/${NC}       Batch audit a whole cert"
echo -e "    ${CYAN}test-audit t.js --stdout | jq${NC}     Pipe the report"
echo ""
